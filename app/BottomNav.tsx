...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-30 bg-[#0e0d12]/90 backdrop-blur-sm border-t border-white/10">
      <div className="mx-auto max-w-2xl flex items-stretch justify-around pb-4 pt-2.5 px-3">
        {TABS.map((tab) => {
          ...
          return (
            <Link
              key={tab.href}
              href={tab.href}
              className={`flex flex-col items-center gap-1.5 min-w-[52px] px-2 py-1.5 rounded-lg transition-colors
                ${active ? 'text-[#8fe0b0]' : 'text-[#8f96a8] hover:text-[#c7c2d3]'}`}
            >
              <span style={{ color: active ? '#8fe0b0' : 'currentColor' }}>{tab.icon}</span>
              <span className={`${mono.className} text-xs font-medium`}>{tab.label}</span>
            </Link>
          )
        })}
      </div>

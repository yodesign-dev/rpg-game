-- Market (buy potions with gold) + use potion (consume from inventory to heal).
-- HP currently only changes as a side effect of fighting in resolve_dungeon_floor
-- (schema.sql) — there is no other way to heal. This adds:
--   1. Seed consumable potion items with a buy_price so they show up in the shop.
--   2. buy_item(): security definer RPC, deducts gold, adds to inventory.
--   3. use_item(): security definer RPC, consumes one unit from inventory, heals HP.
-- Both follow the same auth.uid() ownership check + row lock pattern as
-- resolve_dungeon_floor, since characters/inventory RLS allows an owner to
-- write any column directly — gold/HP must only change through these
-- server-side functions, never a raw client .update() call.

-- Kỹ năng class đợt 2: hồi chiêu + hiệu ứng (lấy cảm hứng DautoRPG, chỉ skill đánh đơn).
--
-- - skills thêm cooldown + effect jsonb; bộ skill mới 5 chủ động + 2 bị động mỗi class
--   (mở ở Lv 1/10/20/30/40, bị động Lv 1/15), seed thẳng trong repo.
-- - simulate_fight: có p_mods.kit thì mỗi lượt dùng skill sẵn sàng mạnh nhất (tính cả
--   thưởng Nhắm Bắn / Băng Vỡ / Kết Liễu), không có thì đánh thường ×1.0. Không có kit →
--   luân phiên a1/a2 như cũ (resolve_dungeon_floor cũ). Chữ ký hàm không đổi.
-- - Hiệu ứng: xuyên giáp, hút máu theo đòn, tự mất HP (chỉ dùng khi HP ≥ 50%), đóng băng
--   (quái mất lượt), độc/thiêu theo lượt, thưởng từ lượt 3, kết liễu, nhiều mũi.
-- - Nhân vật mới tự trang bị skill cấp 1; nhân vật cũ được trang bị lại 2+1 skill mạnh nhất.
-- Mô phỏng (Lv30/50): vùng +10 cấp ngang mức cũ; vùng đúng cấp dễ hơn ~20 trận vì skill
-- mạnh ra đòn ngay lượt đầu.
...
-- 4. Khám phá / Tháp / Nộm tập truyền bộ kỹ năng -----------------------------------

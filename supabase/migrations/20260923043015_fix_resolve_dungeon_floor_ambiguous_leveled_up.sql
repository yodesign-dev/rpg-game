-- Fix "column reference \"leveled_up\" is ambiguous" (42702) in
-- resolve_dungeon_floor. Same root cause as the earlier buy_item/use_item
-- fix: RETURNS TABLE(..., leveled_up boolean, new_level integer, ...)
-- implicitly declares leveled_up/new_level as identifiers in scope for the
-- whole function body. `select leveled_up, new_level into ... from
-- add_experience(...)` collided with those OUT params on every win,
-- breaking the whole dungeon-fight flow with a 400 on click.
-- Return signature is unchanged, so create or replace is enough (no drop).
